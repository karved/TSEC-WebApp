...
class Notification2 extends Component {
  ...
  handleDownloadAttachment = () => {
    ...
    if (this.props.type !== "public") {
      this.props.downloadFile2({ teacherId, fileName: data.msg });
    } else {
  ...
  handleDelete = () => {
    const { teacherId, data } = this.props;
    return firebase.database().ref('/test/principal/Teachers/messages').child(data.msgId).remove()

    
    // let userRef = this.database.ref('test/principal/Teachers/messages').child(data.msgId);
    //  userRef.remove()
...
const mapDispatchToProps = dispatch => {
  return {
    downloadFile2: data => dispatch(downloadFile2(data)),
    downloadFromLink: url => dispatch(downloadFromLink(url))
  };
};

export default connect(null, mapDispatchToProps)(Notification2);
