...
class CreateNotice extends Component {
  ...
  handleGroupCreation = () => {
    ...
    if (classgroups) {
      for (let i = 0; i < classgroups.length; i++) {
        ...
        if (
          addGroup.year === "FE" &&
          addGroup.year === group.year &&
          addGroup.div === group.div
        )
          return M.toast({ html: "Group already Exists!" });
        else if (
          (addGroup.branch === "Computer" || addGroup.branch === "IT") &&
          group.branch === addGroup.branch &&
          group.year === addGroup.year &&
          group.div === addGroup.div
        )
          return M.toast({ html: "Group already Exists!" });
        else if (
          addGroup.branch !== "Computer" &&
          addGroup.branch !== "IT" &&
          group.year === addGroup.year &&
          group.branch === addGroup.branch
        )
    ...
    this.props.createGroup({ group: addGroup, uid }).then(res => {
  ...
  render() {
    ...
    const fab = (
      <Button floating className="red right" >
    ...
    const triggerRichTextModal = (
      <div
        style={{ width: "100%", margin: "20% auto", textAlign: "center" }}
        className="container"
      >
    ...
    const editorModalActions = (
      <div>
        <Button flat onClick={this.handleOnSaveClicked}>
          Save
        </Button>
    ...
    const addClassModalContent = (
      <div className="section">
        <Select
          label="Select Year"
          onChange={event => this.handleGroupSelectChange("year", event)}
        >
          {getYears().map(year => (
        ...
        {addGroup.year !== "FE" && (
          <Select
            label="Select Branch"
            onChange={event => this.handleGroupSelectChange("branch", event)}
          >
        ...
        {getDivs(addGroup.year, addGroup.branch) && (
          <Select
            ...
            onChange={event => this.handleGroupSelectChange("div", event)}
          >
            {getDivs(addGroup.year, addGroup.branch).map(div => (
    ...
    const tabRichText = (
      <Tab
        active={this.state.activeTab === "tab_30"}
        title="Rich"
    ...
    const tabPlainText = (
      <Tab
        active={this.state.activeTab === "tab_31"}
        title="Text"
        className="grey lighten-4"
      >
        <div className="section">
    ...
    const tabMedia = (
      <Tab
        active={this.state.activeTab === "tab_32"}
        title="Media"
        className="grey lighten-4"
      >
        <Dropzone
        ...
        <div className="container section">
          ...
          <div className="center">
            ...
            {this.state.files.length === 0 || this.state.isSubmitClickable ? (
              <Link
                to={{
                  pathname: `/convo/${this.props.uid}`,
                  state: {
                    selectedClass: this.state.selectedClass,
                    classgroups: this.props.classgroups
                  }
                }}
              >
                <Button waves="purple" className="btn-large " onClick={this.handleNoticePush}>
                  Next
                </Button>
              </Link>
            ) : (
  ...
  return {
    classgroups: classgroups
      ? Object.keys(classgroups).map(key => {
          return { key: key, value: classgroups[key] };
        })
      : classgroups,
